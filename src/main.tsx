...
import {
  ...
  MovementPayload,
  MiaoshouOrderAliasJob,
  MiaoshouOrderAliasJobDetail,
  MiaoshouPayload,
  MiaoshouScope,
  ...
  fetchMiaoshou,
  fetchMiaoshouOrderAliasJob,
  fetchMiaoshouOrderAliasJobs,
  fetchOrderAnalysis,
...
function visibleNavItems(user: AuthUser) {
  return navItems.filter((item) => {
    const permissions = [
      ...
      ...("additionalPermissions" in item && Array.isArray(item.additionalPermissions) ? item.additionalPermissions : []),
    ];
    return permissions.some((permission) => (
      permission.startsWith("miaoshou_")
        ? hasMiaoshouCapability(user, permission as MiaoshouCapability)
...
type MiaoshouCapability = "miaoshou_alias" | "miaoshou_listing" | "miaoshou_automation" | "miaoshou_config";

function hasMiaoshouCapability(user: AuthUser, permission: MiaoshouCapability) {
  if (user.permissionOverrides?.deny?.includes(permission)) return false;
  if (hasUserPermission(user, permission)) return true;
  if (!hasUserPermission(user, "miaoshou")) return false;
  return permission === "miaoshou_alias" || hasUserPermission(user, "operations");
}
...
function WecomNotificationCenter({ payload, warehousePayload, onRefresh }: { payload: WecomNotificationPayload | null; warehousePayload: WarehousePayload | null; onRefresh: () => Promise<void> }) {
  ...
  const afterSalesWarehouseRouteCount = Object.values(sceneForm.afterSalesNew?.warehouseRobotIds || {}).reduce((sum, ids) => sum + ids.length, 0);
  const afterSalesNewHasRecipients = Boolean((sceneForm.afterSalesNew?.robotIds || []).length || afterSalesWarehouseRouteCount);
  const afterSalesProgressHasRecipients = Boolean((sceneForm.afterSalesProgress?.robotIds || []).length);
  const collaborationNotificationNeedsEnable = (afterSalesNewHasRecipients && !sceneForm.afterSalesNew?.enabled)
    || (afterSalesProgressHasRecipients && !sceneForm.afterSalesProgress?.enabled);
  ...
  function updateAfterSalesWarehouseRobots(warehouseId: string, robotIds: string[]) {
    ...
    updateAfterSalesNewScene({
      ...
      ...(robotIds.length ? { enabled: true } : {}),
    });
  }

  function enableCollaborationNotifications() {
    setSceneForm((current) => ({
      ...current,
      ...
      <section className="panel wecom-panel">
        ...
        <div className="wecom-scene-grid">
          <article className="wecom-scene-card wecom-after-sales-routing">
            <div className="panel-heading">
              <div>
                <h3>新售后单 / 仓库工单通知对应仓库</h3>
                <p>运营提交后，按处理仓库精确发送到仓库群；消息附带可直接打开对应工单的快捷链接。</p>
              </div>
          ...
          {([
            ...
            return (
              <article className="wecom-scene-card" key={key}>
                ...
                <span className="field-label">推送机器人</span>
                <RobotCheckboxes robots={robots} selected={scene.robotIds || []} onChange={(robotIds) => updateScene(key, { robotIds, ...(key === "afterSalesProgress" && robotIds.length ? { enabled: true } : {}) })} />
    ...
    const denied = user.role === "direct"
      ...
      : user.role === "distributor"
        ...
        : user.role === "guest"
          ...
          : [];
    const configurablePermissions = new Set(permissionCatalog.map((permission) => permission.key));
    const enabled = permissionDraft.enabled.filter((permission) => configurablePermissions.has(permission) && !denied.includes(permission));
...
function normalizedAliasOrderNumber(value: string) {
...
function parseMiaoshouAliasInput(source: string, sourceName = "手动粘贴") {
  ...
  const rows = grid.slice(dataStart).map((cells, index) => ({
    ...
    orderNumber: normalizedAliasOrderNumber(String(cells[orderNumberIndex] || "")),
...
function miaoshouAliasJobStatusLabel(status: string) {
  if (status === "queued") return "排队中";
  if (status === "running") return "匹配中";
  if (status === "verifying") return "二次复查";
  if (status === "completed") return "已完成";
  if (status === "completed_with_warnings") return "完成，需复核";
  if (status === "failed") return "任务失败";
  return status || "未知";
}

function miaoshouAliasJobTone(status: string) {
  if (status === "completed") return "good";
  if (status === "failed") return "danger";
  return "warning";
}

function miaoshouAliasJobRunning(status: string) {
  return ["queued", "running", "verifying"].includes(status);
}

function MiaoshouOrderAliasPanel({
  hasCredentials,
  onJobCreated,
}: {
  ...
  async function importFile(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setDataset(parsed);
      setMessage(`已读取 ${parsed.rows.length} 行、${uniqueOrderCount} 个唯一订单号。提交后由服务器自动拆批处理，无需拆分文件。`);
      setError("");
    } catch (requestError) {
  ...
  async function startMatch() {
    ...
    setError("");
    try {
      const response = await createMiaoshouOrderAliasJob(parsed);
      setMessage("后台任务已创建。现在可以关闭页面，服务器会继续处理。");
      onJobCreated(response.job.id);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "后台任务创建失败");
    } finally {
      setSubmitting(false);
    }
  }
  const uniqueOrderCount = dataset ? new Set(dataset.rows.map((row) => row.orderNumber).filter(Boolean)).size : 0;
  ...
  return (
    <section className="panel miaoshou-alias-panel">
      ...
      {hasCredentials === false ? <div className="notice warning">妙手授权尚未配置：已有缓存仍可匹配，缓存外订单会标记为“查询失败”，不会误标为未匹配。</div> : null}
      {error ? <div className="notice danger">{error}</div> : null}
      ...
      <div className="miaoshou-alias-workspace">
        <div className="miaoshou-alias-input-card">
          ...
          <div className="miaoshou-alias-input-actions">
            ...
            <button className="ghost-button" type="button" disabled={submitting} onClick={() => fileInputRef.current?.click()}><Upload size={15} />上传 CSV / TXT</button>
            <button className="ghost-button" type="button" disabled={submitting} onClick={() => downloadTextFile("妙手店铺别名匹配模板.csv", `\uFEFF${csvCell("平台订单号")}\r\n`, "text/csv;charset=utf-8")}><Download size={15} />下载模板</button>
            <span>{dataset ? `${dataset.sourceName} · ${dataset.rows.length} 行` : "自动识别“平台订单号”或“订单号”列"}</span>
          </div>
        </div>

        <div className="miaoshou-alias-run-card">
          <div className="miaoshou-alias-step"><b>02</b><span><strong>提交后台任务</strong><small>关闭页面后继续运行</small></span></div>
          <div className="miaoshou-alias-rules">
            <span><Check size={15} />每个订单号单独向妙手精确查询</span>
            <span><Check size={15} />查完所有店铺范围才确认“未匹配”</span>
            <span><Check size={15} />未命中与接口异常会自动二次复查</span>
          </div>
          <button className="sync-button miaoshou-alias-run" type="button" disabled={submitting || !sourceText.trim()} onClick={() => void startMatch()}>
            <RefreshCw size={16} className={submitting ? "spinning" : ""} />{submitting ? "正在提交" : "创建后台匹配任务"}
          </button>
          <div className="miaoshou-alias-progress" aria-label="后台任务准备进度"><i style={{ width: dataset ? "100%" : "0%" }} /></div>
          <small>{dataset ? `已准备 ${dataset.rows.length} 行、${uniqueOrderCount} 个唯一订单号。` : "一次导入即可；任务完成后在任务中心下载完整 CSV。"}</small>
        </div>
      </div>
    </section>
  );
...
function MiaoshouOrderAliasTaskCenter({ focusJobId }: { focusJobId?: string }) {
  const [jobs, setJobs] = React.useState<MiaoshouOrderAliasJob[]>([]);
  const [selectedJobId, setSelectedJobId] = React.useState(focusJobId || "");
  const [detail, setDetail] = React.useState<MiaoshouOrderAliasJobDetail | null>(null);
  const [filter, setFilter] = React.useState<"all" | "matched" | "review">("all");
  const [loading, setLoading] = React.useState(true);
  ...
  React.useEffect(() => {
    ...
    async function refresh(silent = false) {
      if (!silent) setLoading(true);
      try {
        const payload = await fetchMiaoshouOrderAliasJobs();
        if (disposed) return;
        setJobs(payload.jobs);
        const nextId = (selectedJobId && payload.jobs.some((job) => job.id === selectedJobId) ? selectedJobId : "")
          || (focusJobId && payload.jobs.some((job) => job.id === focusJobId) ? focusJobId : "")
          || payload.jobs[0]?.id
          || "";
        if (nextId !== selectedJobId) setSelectedJobId(nextId);
        if (nextId) {
          const nextDetail = await fetchMiaoshouOrderAliasJob(nextId);
        ...
        if (!disposed) setError(requestError instanceof Error ? requestError.message : "任务中心读取失败");
      } finally {
  ...
  async function downloadJob(job: MiaoshouOrderAliasJob) {
    ...
    setError("");
    try {
      const blob = await downloadMiaoshouOrderAliasJob(job.id);
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `妙手店铺别名匹配-${job.createdAt.slice(0, 10)}.csv`;
      link.click();
      URL.revokeObjectURL(url);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "匹配结果下载失败");
    } finally {
      setDownloading("");
    }
  }

  const job = detail?.job || jobs.find((entry) => entry.id === selectedJobId) || null;
  const visibleRows = (detail?.previewRows || []).filter((row) => (
    filter === "all"
      || (filter === "matched" ? row.result.status === "matched" : row.result.status !== "matched")
  ));

  return (
    <section className="panel miaoshou-alias-panel miaoshou-alias-task-center">
      <div className="miaoshou-alias-heading">
        <div><p className="eyebrow">Background Task Center</p><h2>订单别名任务中心</h2><span>任务由服务器持续执行；关闭浏览器、退出页面都不会中断。</span></div>
        <span className="status-pill warning">{formatNumber(jobs.filter((entry) => miaoshouAliasJobRunning(entry.status)).length)} 个处理中</span>
      </div>
      {error ? <div className="notice danger">{error}</div> : null}
      <div className="miaoshou-alias-task-layout">
        <aside className="miaoshou-alias-job-list" aria-label="后台匹配任务">
          {jobs.length ? jobs.map((entry) => (
            <button className={entry.id === selectedJobId ? "active" : ""} type="button" key={entry.id} onClick={() => setSelectedJobId(entry.id)}>
              <span><strong>{entry.sourceName}</strong><i className={`status-pill ${miaoshouAliasJobTone(entry.status)}`}>{miaoshouAliasJobStatusLabel(entry.status)}</i></span>
              <small>{formatNumber(entry.rowCount)} 行 · {formatNumber(entry.total)} 个唯一订单</small>
              <div className="miaoshou-alias-progress"><i style={{ width: `${entry.progressPercent || 0}%` }} /></div>
              <small>{entry.progressPercent || 0}% · {formatDateTime(entry.updatedAt)}</small>
            </button>
          )) : <div className="stockup-empty">{loading ? "正在读取后台任务…" : "暂无任务，请先导入订单号并提交。"}</div>}
        </aside>

        <div className="miaoshou-alias-job-detail">
          {job ? <>
            <div className="miaoshou-alias-result-head">
              <div><strong>{job.sourceName}</strong><span>{job.message}</span></div>
              <button className="sync-button" type="button" disabled={!job.downloadReady || downloading === job.id} onClick={() => void downloadJob(job)}><Download size={15} />{downloading === job.id ? "下载中" : job.downloadReady ? "下载完整 CSV" : "完成后可下载"}</button>
            </div>
            {job.directoryWarning ? <div className="notice warning">{job.directoryWarning}</div> : null}
            <div className="miaoshou-alias-summary">
              <article><small>原始行数</small><strong>{formatNumber(job.rowCount)}</strong><span>保留顺序与重复行</span></article>
              <article className="matched"><small>已匹配</small><strong>{formatNumber(job.rowCounts.matched)}</strong><span>订单号与店铺 ID 唯一</span></article>
              <article className="unmatched"><small>未匹配 / 待复核</small><strong>{formatNumber(job.rowCounts.needsReview)}</strong><span>技术异常不会误标未匹配</span></article>
              <article><small>任务进度</small><strong>{job.progressPercent || 0}%</strong><span>{formatNumber(job.processed)}/{formatNumber(job.total)} 个唯一订单</span></article>
            </div>
            <div className="miaoshou-alias-result-head">
              <div className="miaoshou-alias-filters" aria-label="匹配结果筛选">
                <button className={filter === "all" ? "active" : ""} type="button" onClick={() => setFilter("all")}>预览全部</button>
                <button className={filter === "matched" ? "active" : ""} type="button" onClick={() => setFilter("matched")}>已匹配</button>
                <button className={filter === "review" ? "active" : ""} type="button" onClick={() => setFilter("review")}>未匹配 / 待复核</button>
              </div>
              <span className={`status-pill ${miaoshouAliasJobTone(job.status)}`}>{miaoshouAliasJobStatusLabel(job.status)}</span>
            </div>
              ...
              <div className="miaoshou-alias-table">
                ...
                {visibleRows.map((row) => (
                  <article className="miaoshou-alias-row" key={`${row.rowNumber}-${row.orderNumber}`}>
                    ...
                    <span><strong>{row.result.shopAlias || "未匹配"}</strong><small>{row.result.platformShopName || row.result.shopId || "—"}</small></span>
                    <span>{row.result.platform || "—"}<small>{row.result.site || "—"}</small></span>
                    <span><i className={`status-pill ${miaoshouAliasStatusTone(row.result.status)}`}>{miaoshouAliasStatusLabel(row.result.status)}</i></span>
                    <span><strong>{row.result.source === "local_cache" ? "中台缓存" : row.result.source === "miaoshou_live" ? "妙手实时查询" : "—"}</strong><small>{row.result.note}</small></span>
                  </article>
                ))}
              </div>
            </div>
            {job.rowCount > (detail?.previewLimit || 300) ? <div className="miaoshou-alias-limit-note">页面预览前 {detail?.previewLimit || 300} 行；下载 CSV 包含全部 {formatNumber(job.rowCount)} 行。</div> : null}
          </> : <div className="stockup-empty">{loading ? "正在读取任务详情…" : "选择一个任务查看进度和结果。"}</div>}
        </div>
      </div>
    </section>
  );
...
function MiaoshouPage({ currentUser }: { currentUser: AuthUser }) {
  const confirm = useConfirm();
  const canMatchAliases = hasMiaoshouCapability(currentUser, "miaoshou_alias");
  const canManageAutomation = hasMiaoshouCapability(currentUser, "miaoshou_automation");
  const canManageConnection = hasMiaoshouCapability(currentUser, "miaoshou_config");
  const canOpenAutomationWorkspace = canManageAutomation || canManageConnection;
  const [activeWorkspace, setActiveWorkspace] = React.useState<"aliases" | "alias_tasks" | "automation">(() => canMatchAliases ? "aliases" : "automation");
  const [focusedAliasJobId, setFocusedAliasJobId] = React.useState("");
  ...
  React.useEffect(() => {
    if (canOpenAutomationWorkspace) void load();
  }, [canOpenAutomationWorkspace]);

  React.useEffect(() => {
    if ((activeWorkspace === "aliases" || activeWorkspace === "alias_tasks") && !canMatchAliases && canOpenAutomationWorkspace) {
      setActiveWorkspace("automation");
    } else if (activeWorkspace === "automation" && !canOpenAutomationWorkspace && canMatchAliases) {
      setActiveWorkspace("aliases");
    }
  }, [activeWorkspace, canMatchAliases, canOpenAutomationWorkspace]);
  ...
  async function load(silent = false) {
    if (!canOpenAutomationWorkspace) return;
  ...
  async function saveConnectionConfig() {
    await perform("save:connection", () => updateMiaoshouConfig({
      appKey: form.appKey,
      appSecret: form.appSecret,
      scopes: form.scopes,
    }), "妙手连接配置已保存；新凭据只保存在服务端。");
  }
  ...
  async function saveAutomationConfig() {
    ...
    await perform("save:automation", () => updateMiaoshouConfig({
      automationEnabled: form.automationEnabled,
      autoFetchWaybillDefault: form.autoFetchWaybillDefault,
      pollIntervalMinutes: form.pollIntervalMinutes,
      maxPackagesPerRun: form.maxPackagesPerRun,
    }), "自动运单参数已保存。");
  }
    ...
    <main className="miaoshou-page">
      <section className="panel miaoshou-hero">
        <div>
          ...
          <h2>{activeWorkspace === "aliases" ? "订单店铺别名匹配" : activeWorkspace === "alias_tasks" ? "订单别名任务中心" : canManageAutomation ? "店铺自动申请运单号" : "妙手连接配置"}</h2>
          <p>{activeWorkspace === "aliases" ? "导入三方仓的平台后台订单号，逐单精确核验妙手店铺别名。" : activeWorkspace === "alias_tasks" ? "查看服务器后台任务进度；页面关闭后任务继续，完成后可随时下载。" : canManageAutomation ? "逐店开启自动申请。系统只申请运单号并获取面单，不会自动提交平台发货。" : "维护妙手开放平台授权与店铺同步范围，不开放运单执行操作。"}</p>
        </div>
        <div className="miaoshou-hero-actions">
          {activeWorkspace === "aliases" ? <span className="status-pill good">平台订单号精确匹配</span> : activeWorkspace === "alias_tasks" ? <span className="status-pill warning">服务器后台运行</span> : canManageAutomation ? <span className={`status-pill ${config?.automationEnabled ? "good" : "warning"}`}>{config?.automationEnabled ? "自动任务已开启" : "自动任务未开启"}</span> : <span className="status-pill good">连接配置权限</span>}
          {activeWorkspace === "automation" ? <button className="ghost-button" type="button" onClick={() => void load()} disabled={Boolean(busy)}><RefreshCw size={15} className={busy === "load" ? "spinning" : ""} />刷新</button> : null}
          {activeWorkspace === "automation" && canManageAutomation ? <button className="sync-button" type="button" onClick={() => void runNow()} disabled={Boolean(busy) || !payload?.counts.enabledShops}><RefreshCw size={15} className={busy === "run" ? "spinning" : ""} />立即检查</button> : null}
        </div>
      </section>

      <nav className="miaoshou-workspace-tabs" aria-label="妙手 ERP 功能">
        {canMatchAliases ? <button className={activeWorkspace === "aliases" ? "active" : ""} type="button" aria-current={activeWorkspace === "aliases" ? "page" : undefined} onClick={() => setActiveWorkspace("aliases")}><DatabaseZap size={17} /><span><strong>店铺别名匹配</strong><small>导入订单号并创建任务</small></span></button> : null}
        {canMatchAliases ? <button className={activeWorkspace === "alias_tasks" ? "active" : ""} type="button" aria-current={activeWorkspace === "alias_tasks" ? "page" : undefined} onClick={() => setActiveWorkspace("alias_tasks")}><ClipboardList size={17} /><span><strong>任务中心</strong><small>后台进度与结果下载</small></span></button> : null}
        {canOpenAutomationWorkspace ? <button className={activeWorkspace === "automation" ? "active" : ""} type="button" aria-current={activeWorkspace === "automation" ? "page" : undefined} onClick={() => setActiveWorkspace("automation")}><Truck size={17} /><span><strong>{canManageAutomation ? "自动申请运单" : "连接配置"}</strong><small>{canManageAutomation ? "店铺开关与任务记录" : "授权与店铺同步范围"}</small></span></button> : null}
      </nav>

      {error ? <div className="notice danger">{error}</div> : null}
      {message ? <div className="notice success">{message}</div> : null}
      {activeWorkspace === "aliases" && canMatchAliases ? <MiaoshouOrderAliasPanel hasCredentials={config?.hasCredentials} onJobCreated={(jobId) => {
        setFocusedAliasJobId(jobId);
        setActiveWorkspace("alias_tasks");
      }} /> : activeWorkspace === "alias_tasks" && canMatchAliases ? <MiaoshouOrderAliasTaskCenter focusJobId={focusedAliasJobId} /> : <>
      {canManageAutomation && payload?.counts.invalidShops ? <div className="notice warning">检测到 {formatNumber(payload.counts.invalidShops)} 家店铺已解绑或不存在，系统已关闭这些店铺的自动申请并继续处理其他店铺。请在下方店铺列表查看具体店铺和原因。</div> : null}
      <section className="miaoshou-metrics">
        ...
        {canManageAutomation ? <article><small>已同步店铺</small><strong>{formatNumber(payload?.counts.shops || 0)}</strong><span>自动申请 {formatNumber(payload?.counts.enabledShops || 0)} 家 · 失效 {formatNumber(payload?.counts.invalidShops || 0)} 家</span></article> : null}
        {canManageAutomation ? <article><small>已取得运单</small><strong>{formatNumber(payload?.counts.succeeded || 0)}</strong><span>待申请 {formatNumber((payload?.counts.pending || 0) + (payload?.counts.running || 0))}</span></article> : null}
        {canManageAutomation ? <article><small>需要处理</small><strong>{formatNumber((payload?.counts.retryWait || 0) + (payload?.counts.manualCheck || 0))}</strong><span>人工核实 {formatNumber(payload?.counts.manualCheck || 0)}</span></article> : null}
      </section>

      {canManageAutomation ? <section className="miaoshou-flow">
      ...
      {canManageConnection ? <section className="panel miaoshou-config-panel">
        <div className="panel-heading">
          <div><p className="eyebrow">Connection</p><h2>妙手连接配置</h2><span>{config?.lastConnectionTestMessage || "保存授权后检测连接，并同步店铺目录。"}</span></div>
        ...
        <div className="miaoshou-config-actions">
          ...
          <button className="sync-button" type="button" disabled={Boolean(busy)} onClick={() => void saveConnectionConfig()}>{busy === "save:connection" ? "保存中" : "保存连接配置"}</button>
        </div>
      </section> : null}

      {canManageAutomation ? <section className="panel miaoshou-config-panel">
        <div className="panel-heading">
          <div><p className="eyebrow">Automation Scheduler</p><h2>自动运单参数</h2><span>只控制中台自动申请节奏，不允许修改 AppKey 或店铺同步范围。</span></div>
          <span className={`status-pill ${config?.automationEnabled ? "good" : "warning"}`}>{config?.automationEnabled ? "任务已开启" : "任务未开启"}</span>
        </div>
        <div className="miaoshou-config-grid">
        ...
        <div className="miaoshou-config-actions">
          <span>修改只影响自动运单，不会变更妙手授权信息。</span>
          <button className="sync-button" type="button" disabled={Boolean(busy)} onClick={() => void saveAutomationConfig()}>{busy === "save:automation" ? "保存中" : "保存自动运单参数"}</button>
        </div>
      </section> : null}

      {canManageAutomation ? <section className="panel miaoshou-shop-panel">
        <div className="panel-heading">
        ...
        </div> : <div className="stockup-empty">{payload?.counts.shops ? "当前搜索条件下没有店铺。" : canManageConnection ? "尚未同步店铺。先保存授权和平台站点范围，再点击“同步店铺”。" : "尚未同步店铺，请联系具有“妙手连接配置”权限的管理员完成授权和店铺同步。"}</div>}
      </section> : null}

      {canManageAutomation ? <section className="panel miaoshou-task-panel">
...
function ProductDetailModal({
  ...
  const canManageListings = hasMiaoshouCapability(currentUser, "miaoshou_listing")
