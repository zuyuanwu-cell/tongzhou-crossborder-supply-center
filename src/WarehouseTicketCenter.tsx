...
export function WarehouseTicketCenter({ currentUser, initialTicketId = "", initialView = "" }: {
  ...
  async function submit() {
    ...
    setError("");
    try {
      const result = await createWarehouseTicket({ warehouseId, category, priority, relatedOrderNumber, title, description, attachmentIds: attachments.map((item) => item.id), notificationTeamId });
  ...
  return <div className="warehouse-ticket-center">
    ...
    {selectedTicket ? <div className="as-drawer-backdrop" onMouseDown={(event) => event.currentTarget === event.target && setSelectedTicket(null)}><aside className="as-ticket-drawer wt-drawer">
      ...
      <div className="as-drawer-scroll">
        <section className="as-ticket-summary"><span className={`as-status ${statusMeta[selectedTicket.status]?.tone || "muted"}`}>{statusMeta[selectedTicket.status]?.label || selectedTicket.status}</span><div><small>优先级</small><strong>{selectedTicket.priority === "urgent" ? "紧急" : "普通"}</strong></div><div><small>关联订单</small><strong>{selectedTicket.relatedOrderNumber || "未关联"}</strong></div><div><small>进度通知群</small><strong>{selectedTicket.notificationRoute?.teamName || "全局运营群"}</strong></div></section>
